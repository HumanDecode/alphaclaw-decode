...
export const useAppShellController = ({ location = "" } = {}) => {
  ...
  const [branding, setBranding] = useState({
    instanceName: "",
    faviconUrl: "/img/logo.svg",
    documentTitle: "alphaclaw",
  });
  const [acVersion, setAcVersion] = useState(null);
  const [acDecodeVersion, setAcDecodeVersion] = useState(null);
  ...
  useEffect(() => {
    ...
    fetchBranding()
      .then((data) => {
        ...
        setBranding(next);
        document.title = String(next.documentTitle || "alphaclaw");
        const favicon = document.querySelector('link[rel="icon"]');
        if (favicon && next.faviconUrl) favicon.href = next.faviconUrl;
      })
      .catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    const check = async (refresh = false) => {
      try {
        ...
        setAcDecodeVersion(data.currentDecodeVersion || null);
