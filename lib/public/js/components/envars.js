...
const kGroupOrder = ["branding", "ai", "github", "channels", "tools", "custom"];
...
export const Envars = ({ onRestartRequired = () => {} }) => {
  ...
  const handlePaste = (e, field) => {
    ...
    const { mode, pairs } = classifyEnvVarPaste(text, field);
    if (mode === "bulk") {
      e.preventDefault();
  ...
  const handleKeyInput = (raw) => {
    const pairs = parseEnvVarPaste(raw);
    if (pairs.length === 1) {
      setNewKey(pairs[0].key);
      setNewVal(pairs[0].value);
      return;
