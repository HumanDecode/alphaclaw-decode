...
const { hasCanonicalAgentEntries } = require("./config");
...
const createAgentsDomain = ({ fsImpl, OPENCLAW_DIR }) => {
  ...
  const createAgent = (input = {}) => {
    ...
    const promotesCanonicalRosterToFleet =
      hasCanonicalAgentEntries(cfg) && cfg.agents.list.length === 1;
    const soleAgentId = promotesCanonicalRosterToFleet
      ? String(cfg.agents.list[0]?.id || "").trim()
      : "";
    ...
    if (soleAgentId) {
      for (const channel of listConfiguredChannelAccounts({
        fsImpl,
        OPENCLAW_DIR,
        cfg,
      })) {
        for (const account of channel.accounts || []) {
          if (String(account?.boundAgentId || "").trim()) continue;
          appendBindingToConfig({
            cfg,
            agentId: soleAgentId,
            match: { channel: channel.channel, accountId: account.id },
          });
        }
      }
    }
