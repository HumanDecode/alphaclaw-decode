...
const saveConfig = ({ fsImpl, OPENCLAW_DIR, config }) => {
  const persistedConfig = serializeCanonicalAgentEntries(config);
  ensureCodexRuntimePlugin(persistedConfig);
  writeOpenclawConfig({
    fsModule: fsImpl,
    openclawDir: OPENCLAW_DIR,
...
const isObjectSecretRef = (value) =>
  !!value &&
  typeof value === "object" &&
  !Array.isArray(value) &&
  !!String(value.source || "").trim() &&
  !!String(value.id || "").trim();
const getEnvRefKey = (value) => {
  if (isEnvRef(value)) return String(value).trim().slice(2, -1);
  if (isObjectSecretRef(value) && String(value.source).trim() === "env") {
    return String(value.id).trim();
  }
  return "";
};
const isSecretRef = (value) => isEnvRef(value) || isObjectSecretRef(value);
...
const assertActiveChannelTokenEnvVars = ({ cfg, envVars }) => {
  ...
  for (const [provider, providerConfig] of Object.entries(channels)) {
    ...
    for (const [accountId, accountConfig] of accountEntries) {
      ...
      const configuredToken = tokenField ? accountConfig?.[tokenField] : null;
      const envKey =
        getEnvRefKey(configuredToken) ||
        (!isObjectSecretRef(configuredToken)
          ? deriveChannelEnvKey({ provider, accountId })
          : "");
      if (envKey && !String(envMap.get(envKey) || "").trim()) {
        throw new Error(
          `Missing required channel token env var ${envKey} for active channel ${provider}/${accountId}`,
        );
      }
      const extraEnvKeys = deriveChannelExtraEnvKeys({ provider, accountId });
      const extraTokenFields = kChannelExtraTokenFields[provider] || [];
      for (let index = 0; index < extraEnvKeys.length; index += 1) {
        const configuredExtraToken = accountConfig?.[extraTokenFields[index]];
        const extraEnvKey =
          getEnvRefKey(configuredExtraToken) ||
          (!isObjectSecretRef(configuredExtraToken) ? extraEnvKeys[index] : "");
        if (extraEnvKey && !String(envMap.get(extraEnvKey) || "").trim()) {
          throw new Error(
...
const normalizeChannelConfig = ({ provider, channelConfig }) => {
  ...
  if (Object.keys(existingAccounts).length > 0) {
    if (tokenField || extraTokenFields.length > 0) {
      for (const [accountId, accountConfig] of Object.entries(
        ...
        const rawTokenFieldValue = nextAccountConfig[tokenField];
        if (rawTokenFieldValue && !isSecretRef(rawTokenFieldValue)) {
        ...
        extraTokenFields.forEach((fieldName, index) => {
          const rawValue = nextAccountConfig[fieldName];
          if (!rawValue) return;
          if (isSecretRef(rawValue)) return;
  ...
  if (tokenField && defaultAccountConfig[tokenField]) {
    const rawTokenFieldValue = defaultAccountConfig[tokenField];
    if (rawTokenFieldValue && !isSecretRef(rawTokenFieldValue)) {
  ...
  extraTokenFields.forEach((fieldName, index) => {
    const rawValue = defaultAccountConfig[fieldName];
    if (!rawValue) return;
    if (isSecretRef(rawValue)) return;
...
const readPairedCountsByAccount = ({
  ...
  const sqliteEntriesByAccount = readChannelAllowFromByAccount({
    fsImpl,
    openclawDir: OPENCLAW_DIR,
    channel: channelId,
  });
  if (sqliteEntriesByAccount) {
    for (const accountId of counts.keys()) {
      counts.set(
        accountId,
        sqliteEntriesByAccount.get(normalizeChannelAccountId(accountId))?.length || 0,
      );
    }
  }
  const credentialsDir = resolveCredentialsDirPath({ OPENCLAW_DIR });
  try {
    if (
      !sqliteEntriesByAccount &&
      String(channelId || "").trim() !== "whatsapp"
    ) {
      const files = fsImpl
...
const withNormalizedAgentsConfig = ({ OPENCLAW_DIR, cfg }) => {
  ...
  const canonicalRoster = hasCanonicalAgentEntries(nextCfg);
  const existingList = listConfiguredAgents(nextCfg);
  const hasMain = existingList.some(
    (entry) => String(entry.id || "").trim() === kDefaultAgentId,
  );
  const nextList = hasMain || canonicalRoster
    ? existingList
    : [getImplicitMainAgent({ OPENCLAW_DIR, cfg: nextCfg }), ...existingList];
  const listWithSingleDefault = applyDefaultAgentMarkers(nextCfg, nextList);
