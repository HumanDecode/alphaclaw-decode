...
const readModelPrimary = (model) => {
  if (typeof model === "string") return model.trim() || null;
  return String(model?.primary || "").trim() || null;
};

const resolveAgentConfig = (cfg, agentId) => {
  const normalizedAgentId = String(agentId || "").trim();
  if (!normalizedAgentId) return null;
  const entries = isRecord(cfg?.agents?.entries) ? cfg.agents.entries : null;
  if (entries) {
    return Object.prototype.hasOwnProperty.call(entries, normalizedAgentId) &&
      isRecord(entries[normalizedAgentId])
      ? entries[normalizedAgentId]
      : undefined;
  }
  const list = Array.isArray(cfg?.agents?.list) ? cfg.agents.list : null;
  if (list) {
    return list.find(
      (entry) =>
        isRecord(entry) && String(entry.id || "").trim() === normalizedAgentId,
    );
  }
  return normalizedAgentId === kDefaultAgentId ? null : undefined;
};
...
const createAuthProfiles = () => {
  ...
  const getModelConfig = (agentId) => {
    const cfg = loadOpenclawConfig();
    const defaults = cfg.agents?.defaults || {};
    const agentConfig = resolveAgentConfig(cfg, agentId);
    if (agentConfig === undefined) {
      throw new Error(`Unknown agent: ${String(agentId || "").trim()}`);
    }
    ...
    return {
      primary: readModelPrimary(agentConfig?.model) || readModelPrimary(defaults.model),
      configuredModels,
    };
  };

  const setModelConfig = ({ primary, configuredModels }, agentId) => {
    const cfg = loadOpenclawConfig();
    ...
    if (primary !== undefined) {
      if (agentConfig) {
        agentConfig.model = {
          ...(isRecord(agentConfig.model) ? agentConfig.model : {}),
          primary,
        };
      } else {
        if (!isRecord(cfg.agents.defaults.model)) cfg.agents.defaults.model = {};
