...
const kGatewayManagedExitTimeoutMs = 15 * 1000;
let gatewayStderrTail = [];
const expectedExitPids = new Set();
let gatewayRestartInFlight = null;
...
const waitForManagedGatewayExit = (child) =>
  new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(
      () =>
        finish(
          new Error(
            `Managed gateway did not exit within ${kGatewayManagedExitTimeoutMs}ms`,
          ),
        ),
      kGatewayManagedExitTimeoutMs,
    );
...
const stopManagedGatewayChild = async () => {
  ...
  const exitPromise = waitForManagedGatewayExit(child);
  try {
    child.kill("SIGTERM");
  } catch (error) {
    throw new Error(`Could not stop managed gateway: ${error.message}`);
  }
  await exitPromise;
  if (gatewayChild === child) gatewayChild = null;
  return true;
...
const restartGateway = async (reloadEnv) => {
  reloadEnv();
  if (!gatewayRestartInFlight) {
    gatewayRestartInFlight = runGatewayColdStart().finally(() => {
      gatewayRestartInFlight = null;
    });
  }
  await gatewayRestartInFlight;
};
...
const ensureGatewayProxyConfig = (origin) => {
  ...
  try {
    ...
    if (cfg.gateway.controlUi.basePath !== "/openclaw") {
      cfg.gateway.controlUi.basePath = "/openclaw";
      console.log("[alphaclaw] Set gateway Control UI base path: /openclaw");
      changed = true;
    }
